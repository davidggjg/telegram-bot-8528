...
- Pyrogram User-bot (bypasses 20MB limit)
- Telegram Bot (receives files, returns stream links)
- FastAPI streaming with HTTP 206 Range support
- Built-in keep-alive (pings itself every 5 minutes)
- Web dashboard
"""
...
BASE_URL      = os.environ.get("BASE_URL", f"http://localhost:{PORT}")  # e.g. https://your-app.onrender.com
...
async def dashboard():
    ...
    links        = stats["links_generated"]
    last_file    = stats["last_file"] or "—"
    last_ping    = stats["last_ping"] or "—"
...
<body>
  ...
  <div class="grid">
    <div class="card">
      ...
      <div class="label">קבצים שהתקבלו</div>
    </div>
    <div class="card">
      <div class="num">{links}</div>
      <div class="label">קישורים שנוצרו</div>
    </div>
    <div class="card">
      <div class="num" style="font-size:1rem; margin-top:8px">{uptime_since[:10]}</div>
      <div class="label">פעיל מאז</div>
    </div>
  </div>

  <div class="section">
    <h2>📊 מידע נוסף</h2>
    <div class="row"><span class="key">קובץ אחרון</span><span class="val">{last_file}</span></div>
    <div class="row"><span class="key">פינג אחרון</span><span class="val">{last_ping}</span></div>
  ...
  <div class="section">
    ...
    <div class="how">
      ...
      2. הבוט יחזיר לך קישור סטרימינג<br>
      3. הקישור עובד בכל נגן — עם Seek מלא ✅<br><br>
      <strong>פורמט URL ידני:</strong><br>
